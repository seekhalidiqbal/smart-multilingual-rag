...
# --- CUSTOM CSS TO MATCH GRADIO UI ---
st.markdown("""
<style>
    .main-header {background-color: #1e4d8b; padding: 8px 20px; display: flex; align-items: center; gap: 10px; margin: -1rem -1rem 1rem -1rem;}
    .main-header h3 {color: white; margin: 0; font-size: 18px; font-weight: 500;}
    ...
    .stButton>button[kind="secondary"] {background-color: #e0e0e0; color: black; border: none;}
...
with col1:
    st.image("logo.png", width=45) # GitHub me logo.png upload karo
with col2:
    st.markdown("<div class='main-header'><h3>Smart_Multilingual_Multi_Document_AI_RAG_Assistant</h3></div>", unsafe_allow_html=True)
...
with left:
    ...
    with st.container():
        ...
        stats_data = {
            "Metric": ["Total Files", "Total Chunks", "Embedding Model", "LLM Model", "Vector Store"],
            "Value": [len(st.session_state.available_files), len(st.session_state.chunks), "multilingual-MiniLM", "gpt-oss-120b", "FAISS"]
        }
...
st.markdown("<center style='color:gray; font-size:12px;'>© 2026 Department of Computer Science • University of Okara</center>", unsafe_allow_html=True)
